...
void parse_args(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc - 1;)
    {
        ...
        {
            sscanf(argv[i + 1], "%d", &glb_sysconfig.host_send_cc_id);
            i += 2;
        }
        else if (strcmp(argv[i], "-r") == 0)
        {
            sscanf(argv[i + 1], "%d", &glb_sysconfig.host_recv_cc_id);
            i += 2;
        }
        else
        {
    ...
    if (glb_sysconfig.host_send_cc_id < 0 || glb_sysconfig.host_recv_cc_id < 0)
    {
        fprintf(stderr, "Host id must be >= 0\n");
        error = 1;
    }
