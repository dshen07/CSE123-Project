...
struct Host_t {
    ...
    */   
    ...
    int round_trip_num; 
    int csv_out;
    u_int8_t next_seq; // for frame creation
    int LAR;     // for sending window
    int* LFR;     // for receiving windows
    char** currentString; // strings for different hosts
    int* stringLength;
    struct receive_window_slot** receive_window; // receiving windows
    int** receive_window_mask;
    LLnode* incoming_acks_head;
